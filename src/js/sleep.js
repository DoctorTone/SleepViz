...
class Framework extends BaseApp {
  ...
  addGroundPlane() {
    const groundGeom = new THREE.PlaneBufferGeometry(
      APPCONFIG.GROUND_WIDTH,
      APPCONFIG.GROUND_HEIGHT,
      APPCONFIG.GROUND_SEGMENTS
    );
    const gridTexture = this.textureLoader.load("./images/grid.jpg");
    gridTexture.wrapS = gridTexture.wrapT = THREE.RepeatWrapping;
    gridTexture.repeat.set(4, 4);
    const groundMat = new THREE.MeshPhongMaterial({
      color: APPCONFIG.GROUND_MATERIAL,
      map: gridTexture,
    });
  ...
  createBarMaterials() {
    ...
    for (let row = 0; row < APPCONFIG.NUM_ROWS; ++row) {
      barMaterial = new THREE.MeshLambertMaterial({
        color: APPCONFIG.BAR_COLOURS[row],
        flatShading: true,
      });
  ...
  createAttributeMaterials() {
    ...
    for (let attribute = 0; attribute < APPCONFIG.NUM_ATTRIBUTES; ++attribute) {
      barMaterial = new THREE.MeshPhongMaterial({
        color: APPCONFIG.BAR_COLOURS[attribute],
        shininess: 0,
        specular: 0x222222,
      });
  ...
  createGUI() {
    ...
    gui
      .addPanel({ label: "Configuration", width: guiWidth, enable: false })
  ...
  createSceneGroups(monthNumber, visible) {
    ...
    for (
      let attribute = 0;
      attribute < APPCONFIG.attributes.length;
      ++attribute
    ) {
      // Attributes themselves
      currentAttributeGroup = new THREE.Group();
      currentAttributeGroup.name =
        APPCONFIG.attributes[attribute] + monthName + "Group";
      attributeGroups.push(currentAttributeGroup);
      superGroup.add(currentAttributeGroup);

      // Trends
      currentTrendGroup = new THREE.Group();
      currentTrendGroup.name =
        APPCONFIG.attributes[attribute] + "Trend" + monthName + "Group";
      ...
      currentValueGroup.name =
        APPCONFIG.attributes[attribute] + "Values" + monthName + "Group";
  ...
  createBars(monthName) {
    ...
    // Bars
    const barGeom = new THREE.CylinderBufferGeometry(
      APPCONFIG.BAR_RADIUS,
      APPCONFIG.BAR_RADIUS,
      APPCONFIG.BAR_HEIGHT,
      APPCONFIG.BAR_SEGMENTS,
      APPCONFIG.BAR_SEGMENTS
    );
    let barMesh;
    let bars = [];
    ...
    for (let bar = 0; bar < numBars; ++bar) {
      ...
      // Create meshes
      barStartPos.set(
        startPosX + APPCONFIG.BAR_INC_X * bar,
        barStartPos.y,
        barStartPos.z
      );
      for (
        ...
        barMesh.name =
          "bar" + bar + APPCONFIG.attributes[attribute] + monthName;
        ...
        if (bar === 0) {
          ...
          (labelProperty.textColour = "rgba(255, 255, 255, 1.0)"),
            (label = this.labelManager.create(
              "attributeLabel" + APPCONFIG.attributes[attribute],
              APPCONFIG.attributeDisplayNames[attribute],
              labelProperty
            ));
        ...
        if (bar === 0 && attribute === 1) {
          ...
          (labelProperty.textColour = "rgba(0, 0, 0, 1.0)"),
            (label = this.labelManager.create(
              "monthLabel" + APPCONFIG.attributes[attribute] + monthName,
              monthName,
              labelProperty
            ));
          labelGroup.add(label.getSprite());
        ...
        // Lines
        attributeLinePositions[attribute].push(
          barMesh.position.x,
          height * 2,
          barMesh.position.z
        );
        ...
        label = this.labelManager.create(
          "valueLabel" + bar + APPCONFIG.attributes[attribute] + monthName,
          barValue,
          labelProperty
        );
  ...
  adjustCameraPosition() {
    ...
    let bbox = new THREE.Box3().setFromObject(
      MonthlyConfig[this.currentMonthName].attributeGroups[3]
    );
    let bsphere = new THREE.Sphere();
    bbox.getBoundingSphere(bsphere);
    let cameraScale =
      numBars > 10
        ? APPCONFIG.CAMERA_SCALE_LARGE
        : APPCONFIG.CAMERA_SCALE_SMALL;
  ...
  update() {
    ...
    if (this.cameraRotate) {
      ...
      this.tempVec.applyAxisAngle(
        this.camRotAxis,
        this.rotSpeed * this.rotDirection * delta
      );
  ...
  toggleTransparency(year) {
    ...
    if (currentYear) {
      ...
      opacity === 1
        ? (currentYear.children[0].material.opacity = 0.5)
        : (currentYear.children[0].material.opacity = 1.0);
    }
  }

  toggleTrend(attributeName) {
    let currentTrend = this.getObjectByName(
      attributeName + "Trend" + this.currentMonthName + "Group"
    );
    if (currentTrend) {
  ...
  toggleValues(attributeName) {
    let currentAttribute = this.getObjectByName(
      attributeName + "Values" + this.currentMonthName + "Group"
    );
    if (currentAttribute) {
      currentAttribute.visible = !currentAttribute.visible;
    }
  ...
  scaleAttributes(scale) {
    const attributeGroup = this.getObjectByName(
      "SuperGroup" + this.currentMonthName
    );
    if (attributeGroup) {
  ...
  openSideMenu() {
    ...
    document.getElementById("WebGL-Output").style.marginLeft =
      APPCONFIG.SIDE_MENU_WIDTH;
